  ...
  <table class="table table-bordered">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= $u->no_kontrol; ?></td>
        <td><?= $u->role; ?></td>
        <td><?= isset($u->jenjang_id) ? $u->jenjang_id : '-'; ?></td>
        <td><?= isset($u->kepala_sekolah) ? $u->kepala_sekolah : '-'; ?></td>
        <td><?= isset($u->bendahara) ? $u->bendahara : '-'; ?></td>
        <td class="text-center">
          <a href="#" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#edit<?= $u->id; ?>">Edit</a>
          <a href="<?= base_url('admin/user_management/delete/'.$u->id); ?>" onclick="return confirm('Hapus pengguna ini?')" class="btn btn-danger btn-sm">Hapus</a>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
...
<div class="modal fade" id="edit<?= $u->id; ?>" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <form action="<?php echo base_url('user_management/update/'.$u->id); ?>" method="post" class="modal-content">
      <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" 
             value="<?= $this->security->get_csrf_hash(); ?>">
      <div class="modal-header"><h5>Edit Data Sekolah</h5></div>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6 mb-2"><input type="text" name="nama" value="<?= $u->nama; ?>" class="form-control" required></div>
          <div class="col-md-6 mb-2"><input type="text" name="alamat" value="<?= isset($u->alamat)?$u->alamat:''; ?>" class="form-control"></div>
          <div class="col-md-6 mb-2"><input type="email" name="email" value="<?= $u->email; ?>" class="form-control" required></div>
          <div class="col-md-6 mb-2"><input type="text" name="no_kontrol" value="<?= $u->no_kontrol; ?>" class="form-control" required></div>
          ...
          <div class="col-md-3 mb-2">
            <select name="role" class="form-control">
              ...
              <option value="sekolah" <?= ($u->role=='sekolah'?'selected':''); ?>>Sekolah</option>
            </select>
          </div>
          <div class="col-md-3 mb-2">
            <select name="jenjang_id" class="form-control">
              <option value="">Pilih Jenjang</option>
              <?php foreach ($jenjang as $j): ?>
                <option value="<?= $j->id; ?>" <?= ($u->jenjang_id==$j->id?'selected':''); ?>>
                  <?= $j->nama; ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-6 mb-2"><input type="text" name="kepala_sekolah" value="<?= isset($u->kepala_sekolah)?$u->kepala_sekolah:''; ?>" class="form-control"></div>
          <div class="col-md-6 mb-2"><input type="text" name="bendahara" value="<?= isset($u->bendahara)?$u->bendahara:''; ?>" class="form-control"></div>
        </div>
      </div>
